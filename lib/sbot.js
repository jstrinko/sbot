		...
		try {
			...
			var req = protocol.request(options, function(response) {
				...
				response.on('end', function() {
					...
					if (response.statusCode >= 400 && !options.status_ok[response.statusCode]) {
						return callback(Errors.Request_Error(options.url, "Request returned bad status:" + response.statusCode), response);
					}
			...
			req.on('error', function(error) {
				...
				if (error.code == 'ENOTFOUND') {
					error = Errors.Not_Found(server_loc('File'));
				}
				else {
					error = Errors.Request_Error(options.url, error);
				}
				return callback(error);
			});
